...
def _load_locked(pictures_dir: str) -> dict:
    """加载 meta.json（无锁版，调用方必须已持有 _lock）。"""
...
def _save_locked(pictures_dir: str):
    """将当前缓存数据写入 meta.json（无锁版，调用方必须已持有 _lock）。"""
    path = _meta_path(pictures_dir)
...
def load(pictures_dir: str) -> dict:
    ...
    """
    with _lock:
        return _load_locked(pictures_dir)


def save(pictures_dir: str):
    """将当前缓存数据写入 meta.json"""
    with _lock:
        _save_locked(pictures_dir)
...
def set_meta(pictures_dir: str, template_filename: str,
    ...
    """
    with _lock:
        all_meta = _load_locked(pictures_dir)
